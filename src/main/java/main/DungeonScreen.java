...
public class DungeonScreen extends BasicGameState implements InputProviderListener
{
	...
	private List<LevelContext> levels = Lists.newArrayList();
	...
	private boolean heroMove = false;
	...
	public void init(GameContainer container, StateBasedGame game) throws SlickException
	{
		...
		hero = (Hero) new Hero(prepareHero()).register(level, point(1, 1), 0);
		...
		hero.setEventHolder(eventHolder);

		eventHolder.addEvent(new TimeEvent(eventHolder.time, hero, EventType.MOVE));
	...
	private LevelContext createLevel(int depth)
	{
		...
		for (Creature creature : level.getCreatures())
		{
			...
			eventHolder.addEvent(new TimeEvent(eventHolder.time + 1, creature, EventType.MOVE));
		}
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g) throws SlickException
	{
		...
		for (int i = bx; i < bx + Main.NX; i++)
		{
			for (int j = by; j < by + Main.NY; j++)
			{
				if (hero.sees(i, j))
				{
					...
					List<Item> items = level.getItems(i, j);
					if (items.size() > 0)
					{
						// font.drawString((i - bx) * Main.H, (j - by) * Main.H,
        ...
        for (Dwelling c : level.getDwellings())
        {
		...
		if (injector.getInstance(Scenario.class).checkWin(levels))
		{
	...
	boolean makeTick()
	{
		...
		case MOVE:
			...
			if (c == hero)
			{
				return heroMove = true;
			}
			if (!c.isAlive())
			{
				level.getCreatures().remove(c);
			}
			else
			{
				int tm = c.update();
				eventHolder.addEvent(new TimeEvent(eventHolder.time + tm, c, EventType.MOVE));
			}
			break;
		case KILLED:
			if (!c.isAlive())
        ...
        case BIRTH:
            ...
            Optional<Point> availablePoint = Iterables.tryFind(Iterables.concat(asList(((BirthEvent) event).point),
                    point.neighbours()), new Predicate<Point>()
            {
                public boolean apply(Point point)
                {
                    return level.canEnter(point);
                }
            });

            if (availablePoint.isPresent())
            {
                Preconditions.checkArgument(level.canEnter(availablePoint.get()));
                creature.register(level, availablePoint.get(), 1);
                creature.setEventHolder(eventHolder);
	...
	public void controlPressed(Command command)
	{
		...
		if (injector.getInstance(Scenario.class).checkWin(levels)) return;
		...
		else if (command instanceof ActionCommand)
		{
			...
			{
				if (level.getDepth() + 1 >= levels.size()) levels.add(createLevel(level.getDepth() + 1));
				level = levels.get(level.getDepth() + 1);
				hero.enterLevel(level);
	...
	public TimeEvent pollEvent()
	{
		TimeEvent res = eventHolder.poll();
		eventHolder.time = res.getTime();
        for (Dwelling d : level.getDwellings())
        {
            d.birth();
        }
		return res;
