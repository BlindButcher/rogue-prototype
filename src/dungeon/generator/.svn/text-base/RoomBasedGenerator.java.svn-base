...
public class RoomBasedGenerator implements DungeonGenerator
{
	// max size of the map
	private int xmax = 100; // 100 columns
	private int ymax = 100; // 100 rows
	...
	private boolean makeCorridor(int x, int y, int lenght, int direction)
	{
		...
		case 0:
			...
			{
				if (ytemp < 0 || ytemp > ysize) return false; // oh boho, it
																// was!
	...
	private boolean makeRoom(int x, int y, int xlength, int ylength, int direction)
	{
		...
		case 0:
			...
			for (int ytemp = y; ytemp > (y - ylen); ytemp--)
			{
				...
				{
					if (xtemp < 0 || xtemp > xsize) return false;
					if (getCell(xtemp, ytemp) != tileUnused) return false; // no
																			// space
																			// left...
				}
			}
	...
	{
		createDungeon(cols, rows, 500);

		Cell[][] cells = new Cell[rows][cols];
		fillWith(cells, Cell.WALL);
		...
		for (int y = 0; y < ysize; y++)
		{
			for (int x = 0; x < xsize; x++)
			{
				...
				case tileUnused:
					...
					break;
				case tileDownStairs:
					cells[y][x] = Cell.DOWN_STAIRS;
					break;
				case tileUpStairs:
					cells[y][x] = Cell.UP_STAIRS;
					break;
				}
			}
		}
	...
	private static void fillWith(Cell[][] cells, Cell type)
	{
